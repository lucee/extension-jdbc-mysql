component extends="org.lucee.cfml.test.LuceeTestCase" labels="mysqlx" {
	...
	variables.mavenDriverVersionPrefix = "9.7.0";
	...
	private struct function getRegisteredJdbcDriver() {
		...
		try {
			if ( structKeyExists( server, "getMysqlJdbcDriverDefinition" ) ) {
				var jdbc = server.getMysqlJdbcDriverDefinition();
				...
				var ds = server.getDatasource( "mysql" );
	...
	function run( testResults, testBox ) {
		describe( title="MySQL JDBC extension driver version", body=function() {
			it(
				title="reports the MySQL JDBC driver version in use",
				skip=isNotSupported(),
				body=function( currentSpec ) {
					...
					systemOutput( "MySQL JDBC driver info: " & serializeJSON( info ), true );

					expect( dbVersion.recordCount ).toBe( 1 );
					expect( dbVersion.driver_name ).toInclude( "MySQL" );

					if ( resolution.mode eq "maven" ) {
						expect( dbVersion.driver_version ).toInclude( variables.mavenDriverVersionPrefix );
					} else {
						systemOutput( "MySQL JDBC driver loaded via OSGi bundle (#resolution.bundleName# #resolution.bundleVersion#); Maven version assertion skipped", true );
					}
				}
			);
		} );
